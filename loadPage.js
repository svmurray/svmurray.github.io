...
function lb(obj, json)
{
    ...
    if (document.getElementById("imgDiv") == undefined)
    {
        imgDiv = createElementWithStyleProps("div", "", "imgDiv", ["backgroundColor", "position", "top", "left", "height", "width"], ["black", "fixed", "0", "0", "100%", "100%"]);
        ...
        close.onclick = function(){makeHidden(imgDiv);};
        close.data = 1;
        ...
        imgDiv.appendChild(createElementWithStyleProps("div", json.projects[idx].title + "<br />" + json.projects[idx].desc + "<br />" + "1/" + (json.projects[idx].images.length -1), "desc", ["margin", "width", "textAlign", "position", "color", "zIndex", "bottom", "paddingLeft", "backgroundColor", "paddingTop", "paddingRight", "fontSize"], ["0 auto", "36%", "center", "absolute", "white", "1", "1vh", "32%", "rgba(0,0,0,.6)", "1%", "32%", "120%"]));
    ...
    else
    {
        ...
        document.getElementById("desc").innerHTML = json.projects[idx].title + "<br />" + json.projects[idx].desc + "<br />" + "1/" + (json.projects[idx].images.length -1);
        ...
        document.getElementById("close").onclick = function(){makeHidden(imgDiv);};
...
function loadResume(json)
{
    ...
    for (i in json.resume.leadership)
    {
	    if (typeof json.resume.leadership[i].data == "object")
	    {
	        ...
	        for (var j in json.resume.leadership[i].data) 
	        {
	            currList.appendChild(createElement("li", json.resume.leadership[i].data[j].info, "lead" + i + "_" + j, "translatable"));
            }
	        leadList.appendChild(currEl);
	    }
	    else 
	    {
	        leadList.appendChild(createElement("li", json.resume.leadership[i].value, "lead" + i, "translatable"));
	    }
    }
    ...
    for (i in json.resume.skills) 
    {
        skillList.appendChild(createElement("li", json.resume.skills[i].value, "skill" + i, "translatable"));
    }
...
function makeHidden (el) {el.style.visibility = "hidden";}
...
function nextPic(info)
{
    ...
    document.getElementById("desc").innerHTML = info.title + "<br />" + info.desc + "<br />" + imgIdx + "/" + (info.images.length -1);
}
